	...
	>
			...
			<block v-for="(item, index) in menu" :key="index">
				...
				>
					...
					<text
						...
						:style="
							'transform:rotate(' + triangleDeg[index] + 'deg);'
						"
					></text>
				</view>
			</block>
		...
		<block v-for="(page, page_index) in subData" :key="page_index">
			<view
				...
				:class="{
					show: showPage == page_index && page.type !== 'unset',
			...
			>
				...
				>
					...
					<scroll-view
						class="sub-menu-list"
						:class="{
							first: activeMenuArr[page_index].length > 1,
							alone: activeMenuArr[page_index].length <= 1,
						}"
						:scroll-y="true"
						:scroll-into-view="'first_id' + firstScrollInto"
					>
						<block
							v-for="(sub, index) in page.submenu"
							:key="sub.value"
						>
							<view
								class="sub-menu"
								:id="'first_id' + index"
								:class="{
									on: activeMenuArr[page_index][0] == index,
								}"
								@tap="
									selectHierarchyMenu(
										page_index,
				...
				<block v-if="page.type == 'filter'">
					<view class="filter">
						...
						<view class="btn-box">
							<view
								class="reset"
								@tap="resetFilterData(page_index)"
								>重置</view
							>
							<view
								class="submit"
								@tap="setFilterData(page_index)"
								>确定</view
							>
						</view>
					</view>
...
<script>
/**
	 * HM-filterDropdown 下拉菜单
	 * l-filterDropdown 下拉菜单
	 * @description 搜索组件，集成了常见搜索框所需功能，用户可以一键引入，开箱即用。
	 * @tutorial https://www.uviewui.com/components/search.html
	 * @property {Object}			menuData			菜单数据
	 * @property 有空继续完善文档
	 *
	 * @event {Function} confirm 选中的值发生变化后触发
	 * @event {Function} visible-change 弹窗显示隐藏的时候触发, 可能连续触发多次open
	 * @example <HMfilterDropdown
	...
	methods: {
		initMenu() {
			...
			this.activeMenuArr.splice(
				0,
				this.activeMenuArr.length,
				...JSON.parse(JSON.stringify(tmpMenuActiveArr))
			);
			// this.activeMenuArr = JSON.parse(JSON.stringify(tmpMenuActiveArr));
			this.shadowActiveMenuArr = JSON.parse(
				JSON.stringify(tmpMenuActiveArr)
			);
			//加载菜单数据
		...
		setMenuName() {
			for (var i = 0; i < this.activeMenuArr.length; i++) {
				...
				if (
					this.subData[i].type == "hierarchy" ||
					this.subData[i].type == "hierarchy-column"
				) {
		...
		selectHierarchyMenu(
			...
			if (
				...
				this.activeMenuArr.splice(
					...
					1,
					JSON.parse(
						JSON.stringify(this.shadowActiveMenuArr[page_index])
					)
				);
			}
			/////////
			let tmpArr = new Array(this.activeMenuArr[page_index].length).fill(
				null
			);
			this.activeMenuArr[page_index].splice(
				0,
				this.activeMenuArr[page_index].length,
				...tmpArr
			);
			////////
		...
		selectedMemu(page_index, level1_index, level2_index, level3_index) {
			let sub =
				this.subData[page_index].submenu[level1_index].submenu[
					level2_index
				];
			if (this.updateMenuName) {
		...
		hideMenu(isTriggerConfirm) {
			...
			this.$emit("visible-change", false, showPage);
		},
		showMenu(index) {
			if (this.showPage > -1) {
			...
			if (this.menuData[index].type !== "unset") {
				this.showMaskLayer();
			}
			this.$emit("visible-change", true, index);
		},
		//hide遮罩层
		hideMaskLayer() {
		...
		confirm() {
			...
			index.forEach((item, i) => {
				...
				if (typeof item[0] == "object") {
						...
						if (s != null) {
							...
							s.forEach((v, k) => {
								value[i][j][k] =
									v == null ||
									v >=
										this.subData[i].submenu[j].submenu
											.length
										? null
										: this.subData[i].submenu[j].submenu[v]
												.value;
								if (
									this.subData[i].type == "radio" &&
									value[i][j][k] == null
								) {
									value[i][j] = [];
									index[i][j] = [];
					...
					const firstLevel = item[0];
					if (firstLevel === null || firstLevel === undefined) {
						value[i] = [];
						return;
					}
					let submenu = this.subData[i].submenu[firstLevel];
					if (!submenu) {
						value[i] = [];
						return;
					}
			...
			if (this.menu[index].type == "filter") {
				...
				for (let i = 0; i < level; i++) {
					...
					for (let j = 0; j < box.length; j++) {
						if (
							this.shadowActiveMenuArr[index][i].indexOf(j) > -1
						) {
							this.subData[index].submenu[i].submenu[
								j
							].selected = true;
						} else {
							this.subData[index].submenu[i].submenu[
				...
				this.$nextTick(() => {
					setTimeout(() => {
						//滚动到选中项
						this.firstScrollInto = parseInt(
							this.activeMenuArr[index][0]
						);
						this.secondScrollInto = parseInt(
							this.activeMenuArr[index][1]
						);
					}, 0);
				});
				...
				this.$nextTick(() => {
					setTimeout(() => {
						...
						this.thirdScrollInto = parseInt(
							this.activeMenuArr[index][2]
						);
					}, 0);
				});
